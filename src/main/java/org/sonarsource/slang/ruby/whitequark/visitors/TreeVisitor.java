...
public class TreeVisitor<C extends TreeContext> {
  ...
  public void scan(C ctx, Node root) {
    if (root != null) {
      ctx.before(root);
      before(ctx, root);
      visit(ctx, root);
  ...
  private void visit(C ctx, Node node) {
    if (node != null) {
      ctx.enter(node);
      if (consumers != null) {
      ...
      node.getChildren().forEach(child -> visit(ctx, child));
      ctx.leave(node);
    }
  }

  protected void before(C ctx, Node root) {
    // default behaviour is to do nothing
  }

  protected void after(C ctx, Node root) {
    // default behaviour is to do nothing
  }

  public <T extends Node> TreeVisitor<C> register(Class<T> cls, BiConsumer<C, T> visitor) {
    if (consumers == null) {
  ...
  private static class ConsumerFilter<C extends TreeContext, T extends Node> {
    ...
    private void accept(C ctx, Node node) {
